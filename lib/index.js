...
/** 跨平台执行一条 shell 命令：Windows 走 cmd.exe，macOS/Linux 走 /bin/sh */
...
function readLocal() {
  try {
    const out = runCommandSync('dsh --version')
...
/**
 * 执行更新：dsh 通常是 npx 缓存安装，用 `npx -y @deepseek-ai/dsh@latest --version`
 * 触发 npx 重新拉取最新版。跨平台：Windows 走 cmd，macOS/Linux 走 sh。
 * 返回进程句柄用于实时输出。
 */
function startUpdate() {
  ...
  const isWin = process.platform === 'win32'
  const file = isWin ? process.env.ComSpec || 'cmd.exe' : '/bin/sh'
  const args = isWin ? ['/d', '/s', '/c', 'npx -y @deepseek-ai/dsh@latest --version'] : ['-c', 'npx -y @deepseek-ai/dsh@latest --version']
