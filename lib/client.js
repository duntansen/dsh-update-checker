...
exports.apply = function apply(ctx) {
  ...
  function UpdateButton() {
    ...
    const doCheck = () => {
      ...
      return fetch('/dsh-update-check', { headers: { accept: 'application/json' } })
        ...
        .then((data) => {
          ...
          // 自动提醒：版本变化且浏览器支持通知时弹系统通知
          if (data.hasUpdate && lastLocal.current && data.local !== lastLocal.current) {
            setNotified(true)
          }
    ...
    // 请求通知权限（首次有新版本时）
    useEffect(() => {
      if (state.data && state.data.hasUpdate && typeof Notification !== 'undefined' && Notification.permission === 'default') {
    ...
    const toggle = () => {
      const next = !open
      setOpen(next)
      if (next && state.phase === 'idle') doCheck()
    }
    ...
    return h('div', { style: { display: 'inline-flex', position: 'relative' } },
      h('button', {
        ...
        busy ? '…' : (updating ? '⏳' : (badgeNew ? '⬆' : (badgeErr ? '⚠' : '⟳'))),
        badgeNew && h('span', { className: 'upd-dot' })
      ),
  ...
  function UpdateSection() {
    ...
    return h('div', { style: { maxWidth: 560 } },
      ...
      h('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary,#888)', marginBottom: 12 } },
        '自动检查 DSH 是否有新版本，支持一键更新。侧栏底部也有快捷按钮（红色角标 = 有更新）。'),
